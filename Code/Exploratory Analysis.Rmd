...
```{r new variables}
# Variables I don't want to keep for modeling
drop_vars <- c("myasu_course", "sync_st_dt", "sync_end_dt",
               "acad_group", "acad_career", "acad_org", "campus",
               "location", "subject", "catalog_nbr")
...
forum_cleaned <- forum %>%
  mutate(prop_stu_posted = num_stu_with_posts / enrl_total,
         ...
         prop_withdrw = wdwn_ct / enrl_total,
  ...
  select(course_id, strm, session_code, enrl_total, response_rate, has_hallway,
         fac_posts_boc, fac_posts_eoc, total_len_fac_ta_posts, prop_stu_posted,
         avg_gpa, prop_withdrw, instr_score, design_score, posts_per_fac,
         posts_per_student, avg_fac_post_len, upper_division, pass_rate) %>%

...
when the thread was created by a faculty member, so it appears that students perhaps
are more responsive to faculty than their peers. After controlling for outliers,
this amounts to an increase in about 1 student response when the thread is created by faculty. In
...
Faculty exhibit a similar pattern of responsiveness - 90% of threads have no
faculty responses. This alone is not problematic because expecting high 
faculty responsiveness in every thread is unreasonably burdensome. This
pattern persists however (albeit to a lesser degree) in threads with at least one
faculty post. 65% of all threads in this subset still have 
...
ecdf(data[, faculty_responses])(2)

ecdf(data[num_fac_posts > 0, faculty_responses])(1)
ecdf(data[num_fac_posts > 0, faculty_responses])(2)
```

Despite the infrequent nature of faculty responding behavior, students in threads
with at least one faculty response had a relatively high chance of interacting with
faculty.
...
mean(data[between(studs_in_thread, 1, 5) & faculty_responses > 0, prop_stu_responded_to])
...
data %>%
  mutate(fac_resp = if_else(faculty_responses > 0, "Y", "N")) %>%
  filter(studs_in_thread > 0,
         studs_in_thread <= 50) %>%
  group_by(studs_in_thread, fac_resp) %>%
  summarise(prop_stu_responded_to = mean(prop_stu_responded_to)) %>%
  ggplot(aes(studs_in_thread, prop_stu_responded_to, color = factor(fac_resp))) + 
  geom_point() +
  geom_smooth(se = FALSE)

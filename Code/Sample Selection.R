...
#------------------------------------------------------------------------------#
### 0. Setup
libs <- c('readxl', 'tidyverse', 'data.table', 'gridExtra', 'magrittr')
...
forum %<>%
  filter(avg_fac_post_len <= quantile(avg_fac_post_len, 0.95))
#------------------------------------------------------------------------------#
...
forum <- forum %>%
  ...
  select(course_id, has_hallway, enrl_total, fac_post_consistency,
         prop_posts_boc, prop_posts_eoc,
         session_a, avg_grade, instr_score, posts_per_fac,
         posts_per_student, upper_division, avg_fac_post_len, stu_post_consistency) %>%
  mutate_at(vars(session_a, has_hallway, upper_division), as.factor) %>%
  filter_all(all_vars(!is.na(.))) %>%
  filter_if(is.numeric, any_vars(. <= quantile(., 0.95))) %>%

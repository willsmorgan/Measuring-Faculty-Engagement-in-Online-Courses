...
nsr <- data[course == "ENG102" & strm == 2181, .(student_responses, faculty_responded, faculty_owner,
...
# nb_test1 <- MASS::glm.nb(formulas[[4]], nsr,
#                             control = glm.control(maxit = 1000, trace = 3))
...
# lrtest of NB saturated and NB predictors and interactions ====> saturated doesn't add anything
lrtest(nb[[6]], nb[[5]])

# lrtest of NB predictors/interactions against predictors only ===> interactions the way to go
lrtest(nb[[5]], nb[[4]])
```
...
# pure margins
mfx <- margins(model, type = 'response', vce = 'simulation')
summary(mfx)
